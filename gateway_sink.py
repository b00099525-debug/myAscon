...
- Stage A: time scheduling (FIFO) for configured duration
- Transition: Safely moves remaining Stage A packets to Stage B
- Stage B: priority scheduling with Aging to prevent starvation
- Calculates and displays Throughput (KB/s) and End-to-End Delay (ms)
"""
...
def ascon_decrypt(
    ...
    ciphertext: BytesLike, variant: AsconAeadVariant = "Ascon-128", tag_len: int | None = None,
) -> bytes | None:
    p = AEAD_PARAMS[variant]
    if tag_len is None: tag_len = p.tag_len
    assert len(key) == p.key_len and len(nonce) == p.nonce_len and 0 < tag_len <= 16 and len(ciphertext) >= tag_len
...
def bytes_to_state(b: bytes) -> list[int]: return [bytes_to_int(b[8 * w:8 * (w + 1)]) for w in range(5)]
def state_to_bytes(S: list[int]) -> bytes: return b"".join(int_to_bytes(w, 8) for w in S)
def int_to_bytes(integer: int, nbytes: int) -> bytes: return integer.to_bytes(nbytes, "little")
def rotr(val: int, r: int) -> int: return (val >> r) | ((val & ((1 << r) - 1)) << (64 - r))
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            if stage_a_active:
                fifo.append(item)
                print(f"-> [Stage A In] Queued Node: {item.node_id} Seq: {item.seq}")
            else:
                heapq.heappush(heap, (-item.priority_norm, item.arrival_ts, item))
                print(f"-> [Stage B In] Queued Node: {item.node_id} Seq: {item.seq} Base Prio: {item.priority_norm:.3f}")

        except socket.timeout:
            pass 

        # --- Processing & Displaying Metrics ---
        if stage_a_active:
            if fifo:
                processed_item = fifo.pop(0)
                
                # Metric calculations (End-to-End Delay & Throughput)
                delay_ms = (time.time() - processed_item.pkt["ts"]) * 1000.0
                elapsed_time = max(1.0, time.time() - start_ts)
                throughput_kbps = (total_bytes_received / elapsed_time) / 1024.0
                
                print(f"<- [Stage A Out] Node: {processed_item.node_id} Seq: {processed_item.seq} | Delay: {delay_ms:.2f} ms | Throughput: {throughput_kbps:.2f} KB/s")
                if args.decrypt:
            ...
            if heap:
                # Apply Aging to ensure no starvation
                ...
                print(f"<- [Stage B Out] Node: {processed_item.node_id} Seq: {processed_item.seq} | Delay: {delay_ms:.2f} ms | Throughput: {throughput_kbps:.2f} KB/s")
                if args.decrypt:
                    decrypted = try_decrypt(processed_item.pkt)
                    print(f"   Decrypted: {decrypted is not None}")
